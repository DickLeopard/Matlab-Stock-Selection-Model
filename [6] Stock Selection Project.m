...
exlPath = 'D:\UMD\764 Quantitative Investment Strategy\Final Project\data processing'; 
exlFileListInSample =...
    {'1985.xlsx', '1986.xlsx', '1987.xlsx', '1988.xlsx', '1989.xlsx', '1990.xlsx', ...
    '1991.xlsx', '1992.xlsx', '1993.xlsx', '1994.xlsx', '1995.xlsx', ...
    '1996.xlsx', '1997.xlsx', '1998.xlsx',};
...
coefficients = zeros(12, 14);
for fileCount = 1:numel(exlFileListInSample)
        exlFile = exlFileListInSample{fileCount};
    
    [stocks.ticker stocks.id stocks.turn stocks.size ...
        stocks.frev stocks.ltg stocks.sue stocks.bp ...
        stocks.capex stocks.ta stocks.retp stocks.ret2p ...
        stocks.sg stocks.ep stocks.retf]...
        = exlDatafromMorningstar([exlPath '\' exlFile],...
        exlSheet,exlRange);
 % 5.2.3 filters the data
    filteredIndex = stocks.bp > 0 & stocks.eP > 0 &...
        stocks.size >= log(100);
    stocks.ticker = {stocks.ticker{filteredIndex}}';
    stocks.id = {stocks.id{filteredIndex}}';
    stocks.turn = stocks.turn(filteredIndex);
    stocks.size = stocks.size(filteredIndex);
    stocks.frev = stocks.frev(filteredIndex);    
    stocks.ltg = stocks.ltg(filteredIndex);
    stocks.sue = stocks.sue(filteredIndex);
    stocks.bp = stocks.bp(filteredIndex);
    stocks.capex = stocks.capex(filteredIndex);
    stocks.ta = stocks.ta(filteredIndex);
    stocks.retp = stocks.retp(filteredIndex);
    stocks.ret2p = stocks.ret2p(filteredIndex);
    stocks.sg = stocks.sg(filteredIndex);
    stocks.ep = stocks.ep(filteredIndex);
    stocks.retf = stocks.retf(filteredIndex);
    
    % 5.2.2 remove outliers
    stocks = struct2dataset(stocks);
    means = datasetfun(@mean,stocks,'DataVars',{'turn', 'size', ...
        'frev','ltg', 'sue','bp','capex','ta','retp','ret2p','sg','ep','retf'}); 
    stds = datasetfun(@std,stocks,'DataVars',{'turn', 'size', ...
        'frev','ltg', 'sue','bp','capex','ta','retp','ret2p','sg','ep','retf'}); 
    outliersIndx = abs(stocks.turn - means(1)) > (3*stds(1)) |...
        abs(stocks.size - means(2)) > (3*stds(2)) |...
        abs(stocks.frev - means(3)) > (3*stds(3)) |...
        abs(stocks.ltg - means(4)) > (3*stds(4)) |...
        abs(stocks.sue - means(5)) > (3*stds(5)) |...
        abs(stocks.bp - means(6)) > (3*stds(6)) |...
        abs(stocks.capex - means(7)) > (3*stds(7)) |...
        abs(stocks.ta - means(8)) > (3*stds(8)) |...
        abs(stocks.retp - means(9)) > (3*stds(9)) |...
        abs(stocks.ret2p - means(10)) > (3*stds(10)) |...
        abs(stocks.sg - means(11)) > (3*stds(11)) |...
        abs(stocks.ep - means(12)) > (3*stds(12)) |...
        abs(stocks.retf - means(13)) > (3*stds(13));
    stocks(outliersIndx,:) = []; %  ____ stocks left
    
    % 5.2.4 log of the market return of equity and zscores
    stocks.zTurn = zscore(stocks.turn);
    stocks.zSize = zscore(stocks.size);
    stocks.zFrev = zscore(stocks.frev);
    stocks.zLtg = zscore(stocks.ltg);
    stocks.zSue = zscore(stocks.sue);
    stocks.zBp = zscore(stocks.bp);
    stocks.zCapex = zscore(stocks.capex);
    stocks.zTa = zscore(stocks.ta);
    stocks.zRetp = zscore(stocks.retp);
    stocks.zRet2p = zscore(stocks.ret2p);
    stocks.zSg = zscore(stocks.sg);
    stocks.zEp = zscore(stocks.ep);
    stocks.zRetf = zscore(stocks.retf);
    
    % 5.2.5&5.2.6 regression and the output table
    disp(['Regression for file ' exlFile])
    stats = regstats(stocks.zRetf,...
        [stocks.zTurn stocks.zSize stocks.zFrev stocks.zLtg...
        stocks.zSue, stocks.zBp, stocks.zCapex, stocks.zTa, ...
        stocks.zRetp, stocks.zRet2p, stocks.zSg, stocks.zEp],'linear', ...
        {'beta','tstat','fstat','mse','rsquare','adjrsquare','r'});
    regression_table(stats) % may not have the proper table
    disp(' ');
     
    % 5.2.7 save all the beta into a matrix
        coefficients(:,fileCount) = stats.beta; % each column has the beta of one year
    
    clear stocks
    % End of loop over list of in-sample Excel files    
end
clear fileCount exlFile noOutliersIndx filteredIndex stats means stds outliersIndx
